...
TASKS = [
    "agi_eval_lsat_ar", "arc_challenge", "arc_easy",
    "bigbench_cs_algorithms", "bigbench_dyck_languages",
    "bigbench_language_identification", "bigbench_operators",
    "bigbench_qa_wikidata", "bigbench_repeat_copy_logic",
    "boolq", "commonsense_qa", "copa", "coqa",
    "hellaswag", "hellaswag_zeroshot", "jeopardy",
    "lambada_openai", "openbook_qa", "piqa",
    "squad", "winograd", "winogrande",
]
...
DATA = {
    "d8 baseline\n(41.9M)": dict(zip(TASKS, [
        0.0652,  0.0187,  0.2987,
        0.4240,  0.0080,
        0.1705,  0.0952,
        0.0900,  0.0000,
       -0.3579,  0.0350,  0.0000,  0.0920,
        0.0507,  0.0533,  0.0000,
        0.2100,  0.0213,  0.2360,
        0.1040,  0.1282,  0.0680,
    ])),
    "d12 baseline\n(110.1M)": dict(zip(TASKS, [
        0.0272,  0.0453,  0.3813,
        0.4160,  0.0420,
        0.1749,  0.1000,
        0.2860,  0.0000,
       -0.3789,  0.1725,  0.0800,  0.1400,
        0.1547,  0.1627,  0.0060,
        0.2940,  0.1040,  0.3680,
        0.1740,  0.1209,  0.0320,
    ])),
    "d16 baseline\n(234.9M)": dict(zip(TASKS, [
        0.0326,  0.1387,  0.5307,
        0.4160,  0.0700,
        0.1991,  0.1524,
        0.3740,  0.0312,
       -0.2842,  0.1150,  0.1400,  0.2200,
        0.2480,  0.2427,  0.0440,
        0.3380,  0.1547,  0.4240,
        0.2960,  0.1722,  0.0360,
    ])),
    "d20 baseline\n(435.2M)": dict(zip(TASKS, [
        0.0598,  0.1893,  0.5947,
        0.4540,  0.0980,
        0.2013,  0.1714,
        0.4400,  0.0312,
       -0.0632,  0.0725,  0.2800,  0.2480,
        0.3280,  0.3520,  0.0720,
        0.3800,  0.1733,  0.5240,
        0.4180,  0.2601,  0.1280,
    ])),
    "d20 MTP-2\n+ curriculum\n(435.2M)": dict(zip(TASKS, [
        0.0543,  0.2027,  0.6080,
        0.0680,  0.0660,
        0.2035,  0.0143,
        0.0700,  0.0000,
       -0.3368,  0.0250,  0.3200,  0.2260,
        0.3333,  0.3253,  0.0180,
        0.3420,  0.1760,  0.5040,
        0.2580,  0.3187,  0.0760,
    ])),
}
...
ax1.axhline(0.2565, linestyle="--", color="gray", linewidth=1.2, label="GPT-2 ref (0.2565)")
...
for j, (model, col) in enumerate(zip(KEY_MODELS, KEY_COLORS)):
    ...
    ax.bar(x + j * W - W, vals, W, label=model.replace("\n", " "), color=col, alpha=0.85)
...
ax.set_title("Per-Task CORE: d16 baseline vs d20 baseline vs d20 MTP-2+curriculum", fontsize=11)
...
try:
    ...
    runs = api.runs(f"{entity}/nanochat-part4", filters={"display_name": "p4_d20_mtp2_phase2"})
    if not runs:
    ...
    if steps:
        ...
        ax_l.axvline(3669, color="orange", linestyle="--", linewidth=1.2, label="Phase 1→2 boundary")
    ...
    if cores:
        ...
        ax_c.set_title("d20 MTP-2+curriculum — CORE Metric During Training", fontsize=10)
        ax_c.grid(linestyle="--", alpha=0.4)
        
        ax_c.axhline(0.2460, color="green", linestyle="--", linewidth=1.2, label="d20 baseline CORE (0.246)")
        ax_c.axhline(0.1860, color="gray",  linestyle=":",  linewidth=1.2, label="d16 baseline CORE (0.186)")
        ax_c.legend(fontsize=8)

    fig3.suptitle("d20 MTP-2 + Curriculum: Training Dynamics", fontsize=12, fontweight="bold")
    fig3.tight_layout()
