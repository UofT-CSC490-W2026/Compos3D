...
bars = ax1.bar(
    x,
    [CORE_AGGREGATE[c] for c in CONFIGS],
    color=CONFIG_COLORS,
    edgecolor="white",
    linewidth=0.5,
    width=0.55,
...
n_cfgs = len(CONFIGS)
BAR_W = 0.18
offsets = np.linspace(-(n_cfgs - 1) / 2 * BAR_W, (n_cfgs - 1) / 2 * BAR_W, n_cfgs)
...
ax2.set_xticklabels(
    [t.replace("_", "\n") for t in tasks], fontsize=6.5, rotation=0, ha="center"
)
...
ax2.legend(fontsize=9, loc="upper right", title="Config", title_fontsize=8)
