...
def main():
    ...
    if is_hf_model:
        ...
        # Use model-tag in slug so different checkpoints at the same step don't collide
        if args.model_tag:
            tag_slug = args.model_tag.replace("/", "-").replace("\\", "-")
            model_slug = f"{tag_slug}_{meta['step']:06d}"
        else:
