...
Patch for part2_arch: adds --activation and --rope-type arguments.
  --activation relu2   : standard ReLU² MLP (baseline)
  --activation swiglu  : SwiGLU gated MLP with parameter-equivalent intermediate size
  --rope-type rope     : standard Rotary Position Embedding (baseline)
  --rope-type yarn     : YaRN NTK-by-Parts scaled RoPE (Peng et al. 2023)
  --yarn-scale N       : YaRN target context extension factor (default 8.0)
"""
...
parser.add_argument("--activation", type=str, default="relu2", choices=["relu2", "swiglu"],
    help="MLP activation function: 'relu2' (baseline, ReLU²) or 'swiglu' (SwiGLU gated MLP, parameter-equivalent).")
parser.add_argument("--rope-type", type=str, default="rope", choices=["rope", "yarn"],
    help="Positional encoding: 'rope' (standard RoPE, baseline) or 'yarn' (YaRN NTK-by-Parts scaled RoPE).")
...
parser.add_argument("--resume-model-tag", type=str, default=None, help="load checkpoint FROM this model tag (if different from --model-tag which controls where to SAVE)")
parser.add_argument("--load-model-only", action="store_true", help="when resuming, only load model weights — skip optimizer and dataloader state (use this when changing seq-len between phases)")
# Evaluation
...
parser.add_argument("--wandb-project", type=str, default="nanochat", help="WandB project name (default: 'nanochat')")
=======
parser.add_argument(
    "--run",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="load checkpoint FROM this model tag (if different from --model-tag which controls where to SAVE)",
)
parser.add_argument(
    "--load-model-only",
    action="store_true",
    help="when resuming, only load model weights — skip optimizer and dataloader state (use this when changing seq-len between phases)",
)
# Evaluation
parser.add_argument(
    "--eval-every",
...
parser.add_argument(
    ...
    type=str,
    default="nanochat",
    help="WandB project name (default: 'nanochat')",
)
>>>>>>> main
...
wandb_run = (
    DummyWandb()
    if use_dummy_wandb
    else wandb.init(project=args.wandb_project, name=args.run, config=user_config)
)
...
checkpoint_dir = os.path.join(base_dir, "base_checkpoints", output_dirname)  # SAVE destination
=======
...
if resuming:
    # --resume-model-tag lets you load weights from a DIFFERENT model tag than you save to.
    # This is used for context-length curriculum: phase1 saves to part3/d16_ctx512, phase2
    # loads from part3/d16_ctx512 but saves to part3/d16_ctx2048.
    load_dirname = args.resume_model_tag if args.resume_model_tag else output_dirname
    load_checkpoint_dir = os.path.join(base_dir, "base_checkpoints", load_dirname)
    # --load-model-only skips optimizer and dataloader state (use when changing seq-len).
    load_opt = not args.load_model_only
<<<<<<< HEAD
    print0(f"Resuming from step {args.resume_from_step} in '{load_dirname}' -> saving to '{output_dirname}'")
    if args.load_model_only:
        print0("  --load-model-only: skipping optimizer and dataloader state (fresh start)")
    model_data, optimizer_data, meta_data = load_checkpoint(load_checkpoint_dir, args.resume_from_step, device, load_optimizer=load_opt, rank=ddp_rank)
    model.load_state_dict(model_data, strict=True, assign=True)
    del model_data # free up this memory after the copy
=======
    print0(
        f"Resuming from step {args.resume_from_step} in '{load_dirname}' -> saving to '{output_dirname}'"
    )
    if args.load_model_only:
        print0(
            "  --load-model-only: skipping optimizer and dataloader state (fresh start)"
        )
...
if resuming and not args.load_model_only:
...
<<<<<<< HEAD
dataloader_resume_state_dict = None if (not resuming or args.load_model_only) else meta_data["dataloader_state_dict"]
...
dataloader_resume_state_dict = (
    None
    if (not resuming or args.load_model_only)
    else meta_data["dataloader_state_dict"]
)
...
get_report().log(section="Base model training", data=[
    ...
    { # stats about training outcomes
        ...
        "MFU %": f"{mfu:.2f}%" if 'mfu' in dir() else "N/A (no training steps run)",
