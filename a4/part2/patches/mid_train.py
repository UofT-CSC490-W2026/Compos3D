...
Source: nanochat commit 348fbb3 (last commit before midtraining was removed).
Patch for a4/part2:
  --wandb-project  (str, default "nanochat-a4-part2") — configurable W&B project.
  --save-tag       (str, default None) — if set, use this tag as the output directory
                   under mid_checkpoints/ instead of --model-tag.  This decouples the
                   load checkpoint (base_checkpoints/<model-tag>/) from the save path
                   (mid_checkpoints/<save-tag>/) so we can load part3/d20_baseline
                   and save to a4/d20_midtrain without colliding the tags.
"""
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="model tag to LOAD from (base_checkpoints/<model-tag>/)",
)
parser.add_argument(
    "--save-tag",
    type=str,
    default=None,
    help="model tag to SAVE to (mid_checkpoints/<save-tag>/); defaults to --model-tag if unset",
)
parser.add_argument(
...
while True:
    ...
    if master_process and last_step and not args.dry_run:
        save_tag = args.save_tag or args.model_tag or f"d{depth}"
        checkpoint_dir = os.path.join(base_dir, "mid_checkpoints", save_tag)
        save_checkpoint(
            checkpoint_dir,
            step,
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss.backward()
        x, y = next(train_loader)  # prefetch next batch while GPU does forward/backward
        progress = max(
