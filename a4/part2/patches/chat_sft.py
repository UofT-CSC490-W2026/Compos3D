...
Patch for a4/part2:
  Four new CLI arguments:
    --wandb-project       (str,  default "nanochat-a4-part2") — W&B project name,
                          replaces the formerly hardcoded "nanochat-sft".
    --base-model-tag      (str,  default None) — when set, the model and optimizer
                          are loaded from this tag, while --model-tag still controls
                          the save directory (chatsft_checkpoints/<model-tag>/).
                          This decouples the load checkpoint from the save path so
                          the same pretrained base can feed multiple SFT variants.
    --model-source        (str,  default "mid") — checkpoint store to load from:
                            "mid"  → mid_checkpoints/  (Karpathy default: SFT always
                                     loads from the midtrained checkpoint)
                            "base" → base_checkpoints/  (skip midtraining)
    --use-augmented-data  (flag, default False) — when set, appends math-focused
                          datasets to the training mixture:
                            • MetaMathQA (meta-math/MetaMathQA, 395K rows) —
                              augmented GSM8K+MATH with 8 reasoning perspectives
                              (rephrasing, backwards, self-verification, answer-aug).
                              Proven to improve GSM8K +50 pp on 7B-class models.
                            • NuminaMathCoT (AI-MO/NuminaMath-CoT, 100K rows) —
                              competition math (AMC/AIME/Olympiad) with structured
                              chain-of-thought solutions; teaches explicit step-by-step
                              reasoning style that transfers to all math evals.
"""
...
parser.add_argument(
    "--wandb-project", type=str, default="nanochat-a4-part2", help="W&B project name"
)
# Runtime
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="model tag for SAVING the SFT checkpoint (chatsft_checkpoints/<model-tag>/)",
)
parser.add_argument(
    "--base-model-tag",
    type=str,
    default=None,
    help="model tag to LOAD from; if unset, loads from --model-tag",
)
parser.add_argument(
    "--model-source",
    type=str,
    default="mid",
    choices=["base", "mid"],
    help="checkpoint store to load from: 'mid' = mid_checkpoints/ (Karpathy default), 'base' = base_checkpoints/",
)
parser.add_argument(
...
parser.add_argument(
    "--use-augmented-data",
    action="store_true",
    help="append math datasets (MetaMathQA 395K + NuminaMathCoT 100K) to the training data mixture",
)
...
# wandb logging init — project is now configurable via --wandb-project
...
# Resolve which tag to load from vs. save to.
# --base-model-tag decouples load (midtrained/pretrained) from save (SFT output).
# --model-source controls which store: "mid" = mid_checkpoints/ (Karpathy default),
# "base" = base_checkpoints/ (skip midtraining).
load_tag = args.base_model_tag if args.base_model_tag else args.model_tag

# Load the model and tokenizer from the resolved load tag and source
model, tokenizer, meta = load_model(
    args.model_source, device, phase="train", model_tag=load_tag, step=args.model_step
)
...
if args.load_optimizer:
    # Use base_model_tag for optimizer loading if provided (consistent with model loading)
...
# Augmented data mixture: append math-focused datasets for stronger reasoning.
# MetaMathQA (395K rows): augmented GSM8K+MATH with 8 reasoning perspectives —
#   rephrasing, backwards reasoning, self-verification, answer augmentation.
#   Yu et al. (2023) showed +50 pp GSM8K gains on LLaMA-2/Mistral 7B models.
# NuminaMathCoT (100K rows, sub-sampled from 860K): AMC/AIME/Olympiad problems
#   with structured chain-of-thought solutions (Apache 2.0).  Teaches explicit
#   step-by-step reasoning style; powered the 2024 AI Math Olympiad winning entry.
if args.use_augmented_data:
    from tasks.metamath import MetaMathQA
    from tasks.numina_math import NuminaMathCoT

    train_tasks.append(MetaMathQA())  # 395K rows, all of train split
    train_tasks.append(NuminaMathCoT(stop=100_000))  # 100K rows sub-sampled from 860K
    print0(
        "Augmented data: added MetaMathQA (395K) + NuminaMathCoT (100K) to training mixture"
    )

train_dataset = TaskMixture(train_tasks)
mixture_desc = f"MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs}"
if args.use_augmented_data:
    mixture_desc += ", MetaMathQA, NuminaMathCoT"
print0(f"Training mixture: {len(train_dataset):,} rows ({mixture_desc})")
...
tokens_seen = 0  # cumulative tokens processed (for clean x-axis in plots)
...
step = 0
# Running count of padding tokens in the batch (for padding_frac metric)
_pad_tokens_step = 0
_total_tokens_step = 0
while True:
        ...
        wandb_run.log(
            {
                ...
                "val/bpb": val_bpb,
                "val/bpb_min": min_val_bpb,  # running best — for the summary comparison table
            }
        )
        model.train()
    ...
    synchronize()
    t0 = time.time()
    # Count padding in this batch for padding_frac metric (targets == -1 are padding/masked)
    _pad_tokens_step = int((y == -1).sum().item())
    _total_tokens_step = y.numel() * ddp_world_size  # approximate across all ranks
    ...
    padding_frac = _pad_tokens_step / max(_total_tokens_step, 1)
    if step > 10:
    ...
    if step % 10 == 0:
        # Collect actual LR values per optimizer group kind (embedding, unembedding, matrix/muon)
        lrs = {f"train/lr_{g['kind']}": g["lr"] for g in optimizer.param_groups}
        wandb_run.log(
            {
                "step": step,
                "total_training_flops": flops_so_far,
                "total_training_time": total_training_time,
                "train/tokens_seen": tokens_seen,  # clean token-count x-axis for plots
                ...
                "train/padding_frac": padding_frac,  # BOS-bestfit packing efficiency
                **lrs,  # train/lr_embedding, train/lr_matrix, etc.
            }
        )
