...
WANDB_PROJECT = "nanochat-a4-part4"
WANDB_RUN = "a4_d20_rl"
...
TIMEOUT_VERIFY = 60 * 10  # 10 min CPU sanity check
...
# NOTE: nanochat lives under a3/ in this repo (not under a4/).
# We point Modal at the source tree so the container always has the right code.
...
def stage_verify_sft_checkpoint() -> None:
    """
    Cheap CPU-only guardrail before launching expensive RL.

    Verifies that the SFT checkpoint directory exists on the volume and that
    nanochat can resolve a step and open the model/meta files (including
    sharded filenames like model_000966-001.pt).
    """
    _setup_cache()
    volume.reload()
    ckpt_dir = os.path.join(BASE_DIR, "chatsft_checkpoints", SFT_MODEL_TAG)
    print(f"Verifying SFT checkpoint directory: {ckpt_dir}")
    _run(
        "python - <<'PY'\n"
        "import os, glob\n"
        "import torch\n"
        ...
        "print('Resolved last step:', step)\n"
        "device = torch.device('cpu')\n"
        "model_data, _, meta = load_checkpoint(ckpt_dir, step, device)\n"
        "print('Loaded model keys:', len(model_data))\n"
        "print('Loaded meta keys:', sorted(meta.keys()))\n"
        "print('OK')\n"
        "PY"
    )
    print("SFT checkpoint sanity check passed.")
...
def main() -> None:
    w = 64
    print("\n" + "=" * w)
    print("a4/part4: Reinforcement Learning on GSM8K (step-by-step reward shaping)")
    ...
    print("[0] Verifying SFT checkpoint exists (CPU-only, fast)...")
    stage_verify_sft_checkpoint.remote()
