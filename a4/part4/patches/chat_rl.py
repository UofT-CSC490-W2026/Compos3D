...
This is a small patch on top of the a4/part3 version:
- Keep the original binary correctness reward from GSM8K (final answer match).
- Add a formatting / step-by-step shaping bonus so the model is encouraged to
  produce GSM8K-style solutions with intermediate calculator steps:

    ... equation=$<<expression=number>>number \n
    ...
    #### number

The goal is to reward step-by-step reasoning instead of jumping straight to the
...
# -----------------------------------------------------------------------------
# GSM8K-format shaping helpers

_GSM8K_TOOL_RE = re.compile(r"<<[^>]+>>")  # matches <<expr=val>> tool-call chunks
_GSM8K_FINAL_RE = re.compile(r"####\s*\-?[0-9\.\,]+")  # matches the final answer marker


def gsm8k_format_bonus(generated_text: str) -> float:
    """
    Reward structure for "has step-by-step reasoning" in GSM8K format.

    - +0.10 if the model includes a '#### <number>' final answer marker.
    - +0.10 for having at least 1 calculator-style step ('<<...>>').
    - +0.10 extra for having at least 2 such steps (encourages multi-step reasoning).

    Total shaping bonus is capped at 0.30.
    """
    bonus = 0.0
    if _GSM8K_FINAL_RE.search(generated_text) is not None:
        bonus += 0.10
    n_steps = len(_GSM8K_TOOL_RE.findall(generated_text))
    if n_steps >= 1:
        bonus += 0.10
    if n_steps >= 2:
        bonus += 0.10
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        for sample_tokens in generated_token_sequences:
            ...
            # Original reward is binary correctness based on the final "#### number".
            # We add a small bonus for producing GSM8K-style intermediate steps
            # (calculator tool-call format '<<expr=val>>') so the model learns to
            # show step-by-step reasoning rather than jumping to the final answer.
            base_reward = float(train_task.reward(conversation, generated_text))
            reward = base_reward + gsm8k_format_bonus(generated_text)
