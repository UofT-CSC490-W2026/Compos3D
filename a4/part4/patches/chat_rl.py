...
# Results logging for EDA
parser.add_argument(
    "--results-path",
    type=str,
    default=None,
    help="path to append per-problem JSONL records during eval (for EDA)",
)
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        for sample_tokens in generated_token_sequences:
            ...
            reward = (
                args.base_reward_weight * base_reward
                + args.format_reward_weight * format_reward
                + args.arithmetic_reward_weight * arithmetic_reward
            )
...
def get_groundtruth_answer(conversation) -> str:
    assistant_message = conversation["messages"][-1]
    assert assistant_message["role"] == "assistant"
    return extract_text_content(assistant_message["content"])


def get_question_text(conversation) -> str:
...
def compute_format_reward(conversation, generated_text: str) -> float:
    ...
    correct_steps = extract_steps(correct_reasoning)

    generated_reasoning = generated_text.split("###")[0]
    generated_steps = extract_steps(generated_reasoning)

    n_correct = len(correct_steps)
    n_generated = len(generated_steps)

    if n_generated == 0:
        return 0.0
    if n_correct == 0:
        return 1.0 if n_generated == 0 else 0.0
...
def compute_arithmetic_reward(conversation, generated_text: str) -> float:
    """
    Reward in [0, 1] based on:
    1. Whether numbers from the question are reused in the reasoning
    2. Whether intermediate <<expr=result>> equations match the ground truth

    The raw score is normalized by the maximum achievable score for the
    specific example, so the final reward is always in [0, 1].
    """
    question = get_question_text(conversation)
    ...
    question_numbers = set(extract_question_numbers(question))
    generated_numbers = set(extract_question_numbers(generated_reasoning))

    number_score_max = float(len(question_numbers))
    ...
    for step in correct_steps:
        correct_eqs.extend(extract_boxed_equations(step))

    generated_eqs = []
    for step in generated_steps:
        generated_eqs.extend(extract_boxed_equations(step))

    generated_eqs_set = set(eq.strip() for eq in generated_eqs)
    generated_parsed = [parse_equation(eq) for eq in generated_eqs]

    equation_score_max = float(len(correct_eqs))
    for correct_eq in correct_eqs:
        correct_eq = correct_eq.strip()

        if correct_eq in generated_eqs_set:
            raw_reward += 1.0
            continue

        c_lhs, c_rhs = parse_equation(correct_eq)
        partial = 0.0

        for g_lhs, g_rhs in generated_parsed:
            if g_lhs == c_lhs:
                partial = max(partial, 0.5)
            elif c_rhs is not None and g_rhs == c_rhs:
                partial = max(partial, 0.25)

        raw_reward += partial

    max_possible = number_score_max + equation_score_max
    if max_possible <= 0:
        return 0.0

    reward = raw_reward / max_possible
    return min(1.0, max(0.0, reward))
...
for step in range(num_steps):
    ...
    if step % args.eval_every == 0:
        ...
        # Append per-problem results to JSONL file on the volume (master process only)
        if master_process and args.results_path is not None:
            import json as _json

            with open(args.results_path, "a") as _rf:
                for _rec in records:
                    _idx = _rec["idx"]
                    _conv = val_task[_idx]
                    _question = (
                        _conv["messages"][0]["content"] if _conv.get("messages") else ""
                    )
                    _gold = (
                        _conv["messages"][-1]["content"]
                        if _conv.get("messages")
                        else ""
                    )
                    _outcomes = _rec["outcomes"]
                    _correct = any(o["is_correct"] for o in _outcomes)
                    _rf.write(
                        _json.dumps(
                            {
                                "step": step,
                                ...
                                "gold": _gold,
                                "correct": _correct,
                                "num_correct": sum(o["is_correct"] for o in _outcomes),
                                "num_samples": len(_outcomes),
                            }
                        )
                        + "\n"
                    )
