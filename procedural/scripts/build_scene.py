...
_TARGET = (0.0, 0.0, 0.9)
VIEWS = [
    {"name": "overhead", "position": (0.0, 0.0, 8.0), "target": (0.0, 0.0, 0.0)},
    {"name": "front", "position": (0.0, -_ORBIT_R, _ORBIT_H), "target": _TARGET},
    {
        "name": "left",
        "position": (_ORBIT_R * 0.866, _ORBIT_R * 0.5, _ORBIT_H),
        "target": _TARGET,
    },
    {
        "name": "right",
...
def render_views(
    out_dir: Path, resolution: tuple[int, int], samples: int
) -> list[Path]:
...
def render_orbital_video(
    ...
    samples: int,
    orbit_radius: float = 5.5,
    orbit_elevation: float = 3.2,
    target_z: float = 1.0,
) -> list[Path]:
    scene = bpy.context.scene
    scene.render.engine = "CYCLES"
...
def main():
    parser = argparse.ArgumentParser(
        description="Build a 3D scene from a SceneProgram JSON."
    )
    parser.add_argument(
        "--scene_program",
        type=Path,
        required=True,
        help="Path to scene_program.json produced by run-inference",
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="CYCLES samples per video frame (16 = fast)",
    )
    parser.add_argument(
        "--video_frames",
        type=int,
        default=90,
        help="Number of orbital video frames (90 = 3s at 30fps)",
    )
    parser.add_argument(
        "--no_video",
        action="store_true",
        default=False,
        help="Skip video rendering (only render 4 views)",
    )
    ...
    init.apply_gin_configs(
        ["infinigen_examples/configs_nature", "infinigen_examples/configs_indoor"],
    ...
    if not args.no_video:
        frame_paths = render_orbital_video(
            frames_dir=frames_dir,
            n_frames=args.video_frames,
            resolution=res,
            samples=args.video_samples,
            target_z=0.9,
        )
        video_compiled = compile_video(frames_dir, video_path, fps=args.fps)
    elapsed = round(time.time() - t0, 2)
    manifest = {
        "scene_program_path": str(args.scene_program),
        "room_type": room_type,
        "seed": args.seed,
        "resolution": args.resolution,
        "view_samples": args.view_samples,
        "video_samples": args.video_samples,
        "video_frames": args.video_frames,
        "rendered_views": [str(p) for p in rendered_views],
        "video_frames_dir": str(frames_dir) if frame_paths else None,
        "video_path": str(video_path) if video_compiled else None,
