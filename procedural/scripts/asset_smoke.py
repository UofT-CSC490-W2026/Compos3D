...
def adjust_cam_distance_autofit(asset, camera, margin=0.01, percent=0.999):
    ...
    bbox_pts_world = (
        np.array(asset.matrix_world)
        @ np.c_[bbox_pts_local, np.ones(len(bbox_pts_local))].T
    ).T[:, :3]
...
def build_object(FI, args):
    factory_cls = FI.CLS
    assert args.param_mode in FI.PARAM_OPTS, (
        f"param_mode '{args.param_mode}' not in PARAM_OPTS {list(FI.PARAM_OPTS.keys())}"
    )
...
def main():
    ...
    if args.factory_name not in lib.LIB_MAP:
        raise ValueError(
            f"factory_name '{args.factory_name}' not in LIB_MAP. Available: {sorted(lib.LIB_MAP.keys())}"
        )
    ...
    manifest = {
        ...
        "render_exists": render_path.exists() if render_path else False,
