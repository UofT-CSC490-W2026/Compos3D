...
class LampFactory(AssetFactory):
    ...
    def __init__(self, params):
        ...
        self.params = {'StandRadius': float(params.get('StandRadius', 0.01)), 'BaseRadius': float(params.get('BaseRadius', 0.1)), 'BaseHeight': float(params.get('BaseHeight', 0.02)), 'ShadeHeight': float(params.get('ShadeHeight', 0.2)), 'HeadTopRadius': float(params.get('HeadTopRadius', 0.1)), 'HeadBotRadius': float(params.get('HeadBotRadius', 0.12)), 'ReverseLamp': bool(params.get('ReverseLamp', True)), 'RackThickness': float(params.get('RackThickness', 0.002)), 'CurvePoint1': _ensure_vector(params.get('CurvePoint1', (0.0, 0.0, 0.0))), 'CurvePoint2': _ensure_vector(params.get('CurvePoint2', (0.0, 0.0, 0.2))), 'CurvePoint3': _ensure_vector(params.get('CurvePoint3', (0.0, 0.0, 0.3)))}
        ...
        self.params['LampshadeMaterial'] = lampshade_gen() if callable(lampshade_gen) else lampshade_gen
