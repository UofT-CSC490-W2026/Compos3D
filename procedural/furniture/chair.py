...
from infinigen.assets.utils.decorate import read_co, read_edge_center, read_edge_direction, remove_edges, remove_vertices, select_edges, solidify, subsurf, write_attribute, write_co
...
class ChairFactory(AssetFactory):
    ...
    def __init__(self, params):
        ...
        if arm_mid is not None and hasattr(arm_mid, '__iter__') and (len(arm_mid) >= 3):
            self.arm_mid = np.array([float(arm_mid[0]), float(arm_mid[1]), float(arm_mid[2])])
        else:
            self.arm_mid = np.array([0.0, 0.03, -0.03])
        arm_profile = p.get('arm_profile')
        if arm_profile is not None and hasattr(arm_profile, '__iter__') and (len(arm_profile) >= 2):
        ...
        self.back_bridge_profile_shape_factor = float(p.get('back_bridge_profile_shape_factor', 0.2))
        ...
        if limb_gen is None:
            ...
            self.limb_surface_material_gen = limb_gen if callable(limb_gen) else lambda: limb_gen
        surface_gen = p.get('surface')
        if surface_gen is None:
            self.surface_material_gen = material_assignments.furniture_hard_surface[0][0]()
        else:
            self.surface_material_gen = surface_gen if callable(surface_gen) else lambda: surface_gen
        panel_gen = p.get('panel_surface')
        if panel_gen is None:
            self.panel_surface_material_gen = material_assignments.furniture_hard_surface[0][0]()
        else:
            self.panel_surface_material_gen = panel_gen if callable(panel_gen) else lambda: panel_gen

    def create_placeholder(self, **kwargs) -> bpy.types.Object:
        obj = new_bbox(-self.width / 2 - max(self.leg_x_offset, self.back_x_offset), self.width / 2 + max(self.leg_x_offset, self.back_x_offset), -self.size - self.leg_y_offset[1] - self.leg_thickness * 0.5, max(self.leg_y_offset[0], self.back_y_offset), -self.leg_height, self.back_height * 1.2)
    ...
    def make_seat(self):
        x_anchors = np.array([0, 0.1, 1, self.seat_mid_x, self.seat_back, 0]) * self.width / 2
        y_anchors = np.array([-self.seat_front, -self.seat_front, -1, -self.seat_mid, 0, 0]) * self.size
    ...
    def make_legs(self):
        leg_starts = np.array([[-self.seat_back, 0, 0], [-1, -1, 0], [1, -1, 0], [self.seat_back, 0, 0]]) * np.array([[self.width / 2, self.size, 0]])
    ...
    def make_limb(self, leg_ends, leg_starts):
        ...
        for leg_start, leg_end in zip(leg_starts, leg_ends):
            ...
            limb.location = np.array([1 if leg_start[0] < 0 else -1, 1 if leg_start[1] < -self.size / 2 else -1, 0]) * self.leg_thickness / 2
    ...
    def make_back_decors(self, backs, finalize=True):
        ...
        with butil.ViewportMode(obj, 'EDIT'):
            ...
            for z_min, z_max in self.back_profile:
                select_edges(obj, (z_min * self.back_height <= center[:, -1]) & (center[:, -1] <= z_max * self.back_height))
                bpy.ops.mesh.bridge_edge_loops(number_cuts=64, interpolation='LINEAR', smoothness=smoothness, profile_shape_factor=profile_shape_factor)
        ...
        if self.back_type == 'vertical-bar':
            ...
            with butil.ViewportMode(other, 'EDIT'):
                bpy.ops.mesh.select_mode(type='EDGE')
                bpy.ops.mesh.select_all(action='SELECT')
                bpy.ops.mesh.bridge_edge_loops(number_cuts=self.back_vertical_cuts, interpolation='LINEAR', smoothness=smoothness, profile_shape_factor=profile_shape_factor)
            ...
            remove_vertices(other, lambda x, y, z: z > (self.back_profile[0][0] + self.back_profile[0][1]) * self.back_height / 2)
    ...
    def make_arms(self, base, backs):
        ...
        for start_pt, end_pt in zip([start, start_], [end, end_]):
            mid = np.array([end_pt[0] + self.arm_mid[0] * (-1 if end_pt[0] > 0 else 1), end_pt[1] + self.arm_mid[1], start_pt[2] + self.arm_mid[2]])
            arm = align_bezier(np.stack([start_pt, mid, end_pt], -1), np.array([[end_pt[0] - start_pt[0], end_pt[1] - start_pt[1], 0], [0, 1 / np.sqrt(2), 1 / np.sqrt(2)], [0, 0, 1]]), [1, *self.arm_profile, 1])
            if self.is_leg_round:
                surface.add_geomod(arm, geo_radius, apply=True, input_args=[self.arm_thickness / 2, 32], input_kwargs={'to_align_tilt': False})
            else:
                with butil.ViewportMode(arm, 'EDIT'):
                    bpy.ops.mesh.select_all(action='SELECT')
                    bpy.ops.mesh.extrude_edges_move(TRANSFORM_OT_translate={'value': (self.arm_thickness if end_pt[0] < 0 else -self.arm_thickness, 0, 0)})
