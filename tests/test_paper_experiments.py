...
def test_utils(tmp_path):
    ...
    res = write_experiment_config(base_config_path=bconf, output_path=outconf, overrides={"a": {"c": 2}})
    assert res == outconf
    assert _read_json(outconf) == {"a": {"b": 1, "c": 2}}
    
    # _maybe_materialize_config
    assert _maybe_materialize_config(config_path=None, config_overrides=None) == (None, None)
    assert _maybe_materialize_config(config_path=bconf, config_overrides=None) == (bconf, None)
...
def test_run_generation_benchmark(monkeypatch, tmp_path):
    ...
    benchmark.write_text(json.dumps({"examples": [
        {"example_id": "1", "prompt": "a", "room_type": "dining_room", "required_assets": ["table"]},
    ...
    def fake_run_inference(output_dir, **kw):
        ...
        (output_dir / "inference_manifest.json").write_text('{"selected_hypothesis_ids": ["h1"], "render_scene": true}')
        (output_dir / "scene_program.json").write_text('{"room_type": "dining_room", "assets": [], "image_paths": ["img1.png"]}')
        (output_dir / "critic_score.json").write_text('{"validity": 1.0, "prompt_adherence": 1.0, "asset_precision": 1.0, "asset_recall": 1.0, "room_match": 1.0, "overall": 1.0}')
        
    monkeypatch.setattr(mod, "run_vertical_inference", fake_run_inference)
    monkeypatch.setattr(mod, "build_program_metric_row", lambda **kw: {"exact_room_type_accuracy": 1.0})
    
    with pytest.raises(ValueError):
        run_generation_benchmark(benchmark_path=benchmark, output_dir=out_dir, method_name="m")
...
def test_judge_generation_pairwise(monkeypatch, tmp_path):
    ...
    a_res.write_text('{"example_id": "1", "image_paths": ["a1.png"], "prompt": "a", "method_name": "A"}\n'
                     '{"example_id": "2", "image_paths": ["a2.png"], "prompt": "b", "method_name": "A"}')
                     
    b_res = tmp_path / "b.jsonl"
    b_res.write_text('{"example_id": "1", "image_paths": ["b1.png"], "prompt": "a", "method_name": "B"}\n'
    ...
    class FakeJudge:
        ...
        def judge_generation_pair(self, **kw):
            return {"asset_selection": "A", "layout_coherence": "B", "overall_preference": "tie"}
    ...
    class FlakyJudge:
        def __init__(self, config): self.attempts = 0
        def judge_generation_pair(self, **kw):
            self.attempts += 1
            if self.attempts < 2:
    ...
    existing_out.mkdir()
    (existing_out / "pairwise_generation.jsonl").write_text('{"example_id": "1", "asset_selection": "A", "method_a": "A"}\n{"example_id": "2", "asset_selection": "B", "method_a": "A"}')
    res = judge_generation_pairwise(
        method_a_results_path=a_res,
        method_b_results_path=b_res,
    ...
    existing_out4.mkdir()
    (existing_out4 / "pairwise_generation.jsonl").write_text('{"example_id": "1", "asset_selection": "A", "method_a": "A"}')
    monkeypatch.setattr(mod, "BedrockPairwiseJudge", FakeJudge)
    res = judge_generation_pairwise(
        method_a_results_path=a_res,
        method_b_results_path=b_res,
...
def test_run_edit_benchmark(monkeypatch, tmp_path):
    ...
    pairs_path.write_text(json.dumps({
        "pair_id": "p1", "base_prompt": "a", "edited_prompt": "b", 
        "base_example_id": "b1", "edited_example_id": "e1",
        "edit_instruction": "do it", "edit_type": "add",
        "base_required_assets": [], "base_expected_asset_counts": {},
        "edited_required_assets": [], "edited_expected_asset_counts": {},
    }) + "\n")
    
    def fake_run_inference(output_dir, **kw):
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "inference_manifest.json").write_text('{}')
        (output_dir / "scene_program.json").write_text('{"room_type": "dining_room", "assets": []}')
        (output_dir / "critic_score.json").write_text('{"validity": 1.0, "overall": 1.0, "used_image_paths": ["i.png"]}')
        
    monkeypatch.setattr(mod, "run_vertical_inference", fake_run_inference)
    monkeypatch.setattr(mod, "build_program_metric_row", lambda **kw: {"exact_room_type_accuracy": 1.0})
    monkeypatch.setattr(mod, "compute_edit_program_metrics", lambda **kw: {"delta_asset_precision": 1.0})
    ...
    assert summarize_edit_rows([])["metrics"] == {"delta_asset_precision": None, "base_overall": None, "clip_directional_similarity": None, "vlm_edit_success": None, "delta_asset_recall": None, "delta_asset_f1": None, "delta_count_l1": None, "unchanged_asset_retention_rate": None, "edited_overall": None, "vlm_preservation": None, "vlm_overall": None}
...
def test_export_human_study_sheets(tmp_path):
    ...
    edit_res.write_text(json.dumps({
        "pair_id": "p1", "edit_type": "add", "base_prompt": "a", "edited_prompt": "b",
        "edit_instruction": "c", "base_image_paths": ["b.png"], "edited_image_paths": ["e.png"]
    }))
    
    out_edit = tmp_path / "edit.csv"
    export_edit_human_study_sheet(
        editing_results_path=edit_res,
        output_csv_path=out_edit
    )
