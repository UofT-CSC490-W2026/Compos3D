...
def test_bedrock_pairwise_judge_run_json_prompt(monkeypatch, tmp_path):
    ...
    with pytest.raises(CriticUnavailableError, match="credentials are unavailable or expired"):
...
def test_bedrock_pairwise_judge_generation_pair(monkeypatch):
    judge = BedrockPairwiseJudge(CriticConfig())
    monkeypatch.setattr(judge, "_run_json_prompt", lambda **kwargs: {"layout_coherence": "A"})
...
def test_bedrock_pairwise_judge_edit_pair(monkeypatch):
    judge = BedrockPairwiseJudge(CriticConfig())
    monkeypatch.setattr(judge, "_run_json_prompt", lambda **kwargs: {"edit_success": 1.0})
...
def test_clip_directional_similarity(monkeypatch, tmp_path):
    ...
    class DummyModel:
        ...
        def encode_image(self, batch):
            class DummyTensor:
                ...
                def norm(self, **kwargs): return DummyTensor(1.0)
                def mean(self, **kwargs): return self
                def __truediv__(self, other): return self
                def __sub__(self, other): return DummyTensor(self.val - other.val)
                def item(self): return float(self.val)
                def sum(self): return self
                def __mul__(self, other): return DummyTensor(self.val * other.val)
        ...
        def encode_text(self, tokenized):
            class DummyTensor:
                ...
                def norm(self, **kwargs): return DummyTensor(1.0)
                def __truediv__(self, other): return self
                def __getitem__(self, idx): return DummyTensor(float(idx))
                def __sub__(self, other): return DummyTensor(self.val - other.val)
                def item(self): return float(self.val)
                def __mul__(self, other): return DummyTensor(self.val * other.val)
                def sum(self): return self
    ...
    monkeypatch.setattr(mod, "_load_open_clip_components", lambda: (DummyModel(), lambda x: x, lambda x: DummyTokenizer(x), DummyTorch(), "cpu"))
    ...
    class DummyModel2(DummyModel):
        def encode_image(self, batch):
            class DummyTensor2:
                ...
                def norm(self, **kwargs): return DummyTensor2(0.0)
                def mean(self, **kwargs): return self
                def __truediv__(self, other): return self
                def __sub__(self, other): return DummyTensor2(0.0)
                def item(self): return float(self.val)
            return DummyTensor2(1.0)
    monkeypatch.setattr(mod, "_load_open_clip_components", lambda: (DummyModel2(), lambda x: x, lambda x: DummyTokenizer(x), DummyTorch(), "cpu"))
    assert mod.clip_directional_similarity(before_image_paths=[a], after_image_paths=[b], before_text="a", after_text="b") is None
